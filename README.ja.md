...
shipkeyは環境変数名からプロバイダーを自動検出します。40以上のプロバイダーにセットアップガイド付きで対応しています。

| カテゴリ | プロバイダー | 環境変数パターン |
|----------|-------------|-----------------|
...
| **開発プラットフォーム** | Expo | `EXPO_*` |
| | Daytona | `DAYTONA_*` |
| **インフラ** | Cloudflare | `CLOUDFLARE_*`, `R2_*` |
| | AWS | `AWS_*`, `EC2_*` |
| | Vercel | `VERCEL_*` |
| | Fly | `FLY_*` |
| | npm | `NPM_*` |
...
| **アナリティクス** | Plausible | `PLAUSIBLE_*` |
| **フォーム** | Formspree | `FORMSPREE_*` |
| **セキュリティ** | Turnstile | `TURNSTILE_*` |
| **サンドボックス** | Sandbank | `SANDBANK_*`, `SANDBOX_*` |

認識されない変数は **General** グループに分類されます。新しいプロバイダーの追加は [issue](https://github.com/chekusu/shipkey/issues) でリクエストできます。
...
```bash
shipkey push                   # prod 環境をプッシュ（デフォルト）
shipkey push -e dev            # dev 環境をプッシュ
...
```bash
shipkey pull                   # prod 環境を取得（デフォルト）
shipkey pull -e dev            # dev 環境を取得
...
| 環境 | 通常プロジェクト | Cloudflare Workers |
|------|-----------------|-------------------|
